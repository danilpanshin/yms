...
@section('title', 'Экспедиторы')

@section('content')
    @php
        $add_route = 'supplier.expeditor.add_post';
        $edit_route = 'supplier.expeditor.edit_post';
        $delete_route = 'supplier.expeditor.delete_post';
        $restore_route = 'supplier.expeditor.restore_post';
        $view_route = 'supplier.expeditor.one';
        function delete_confirm($name, $id): void { echo "Вы действительно хотите перенести в архив экспедитора {$name} c ID {$id}"; }
        function restore_confirm($name, $id): void { echo "Вы действительно хотите восстановить экспедитора {$name} c ID {$id}"; }
    ...
    <div class="admin_users_section">
        <div class="row">
            <div class="col-12 col-sm-6 text-start">
                ...
                @else
                    <a href="{{ route('supplier.expeditor.with_trashed') }}" class="btn btn-warning">Показать архив</a>
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            @include('supplier.expeditor.add_form')
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Закрыть</button>
                            <button type="button" class="btn btn-primary" onclick="$('.addFormExpeditor').submit();">Сохранить</button>
                        </div>
                    </div>
                </div>
            </div>
...
{{--                        <td>@foreach($row['roles'] as $role_row) <span class="badge rounded-pill bg-info">{{ $role_row['name'] }}</span> @endforeach</td>--}}
                        <td style="width: 160px;">
                            <form action="{{ route($delete_route, $row['id']) }}" method="post" class="expeditor_deleteForm_{{ $row['id'] }}">
                                @csrf
                            </form>
                            <form action="{{ route($restore_route, $row['id']) }}" method="post" class="expeditor_restoreForm_{{ $row['id'] }}">
                                @csrf
                            </form>
                            ...
                            @if($row['deleted_at'])
                                <a title="Восстановить" class="btn btn-sm btn-danger ms-4" onclick="if(confirm('@php restore_confirm($row['name'], $row['id']) @endphp')){ $('.expeditor_restoreForm_{{ $row['id'] }}').submit(); }">
                                    <i class="bi bi-repeat"></i>
                                </a>
                            @else
                                <a title="В архив" class="btn btn-sm btn-success ms-4" onclick="if(confirm('@php delete_confirm($row['name'], $row['id']) @endphp')){ $('.expeditor_deleteForm_{{ $row['id'] }}').submit(); }">
