...
@section('content')
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.dictionary.car_type.add_post') }}" method="post" class="addForm">
                            @csrf
                            <div class="mb-3">
    ...
    <div class="row mt-2">
        <table class="table table-condensed table-striped-columns rounded-3 overflow-hidden">
            ...
            <tbody>
            @foreach($car_types as $row)
                <tr>
                    <td style="width: 60px;">{{ $row['id'] }}</td>
                    <td style="width: 400px;">{{ $row['name'] }}</td>
                    <td>{{ $row['comment'] }}</td>
                    <td style="width: 160px;">
                        <form action="{{ route('admin.dictionary.car_type.delete_post', $row['id']) }}" method="post" class="deleteForm">
                            @csrf
                        </form>
...
@section('script')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('admin.dictionary.car_type.edit_post') }}" id="editModal" method="post" class="editForm">
                        @csrf
    ...
    <script>
        $('.view_btn').on('click', function(){
            ...
            $.ajax({
                url: '{{ route('admin.dictionary.car_type.one') }}/' + id,
